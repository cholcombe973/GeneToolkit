...
public abstract class Worker implements NetworkListener{
   ...
   * <p>A bug will show itself with this method if the first packet is smaller than 16 bytes.  If it is
   * the collectData method won't read the correct size to wait for.</p>
   * <p>
   * collectData takes the first 8 bytes and read as a long, and multiples it by the 2nd 8 bytes read as a long
   * to come up with the total expected packet size.
   *
   * </p>
  ...
  protected void collectData(ByteBuffer newData,SocketChannel callback){
    if(firstCall){
      firstCall = false;
      int numberOfRows = (int)newData.getLong(9);
      int numbersPerRow = (int)newData.getLong(17);

      //allocate internal buffer, protocol data + (numberOfRows * numbersPerRow * 8 bytes per double)
      internalBuffer = ByteBuffer.allocate(25+(numberOfRows*numbersPerRow*8));
